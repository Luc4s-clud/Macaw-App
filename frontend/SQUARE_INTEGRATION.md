...
## Onde cadastrar os itens do menu

Os itens **não** são cadastrados no seu código — eles ficam no **catálogo do Square**. O backend só lê esse catálogo e devolve para o site.

### Sandbox (testes)

1. Acesse o [Square Developer Dashboard](https://developer.squareup.com/apps).
2. Abra sua aplicação e vá em **Sandbox** (ou **Test**).
3. Use o **Square Sandbox Dashboard** (link para gerenciar a conta de teste).
4. Lá você cria **Items** (produtos): nome, preço, descrição, categoria, imagens.
5. Também dá para importar em massa por planilha (bulk import).

Documentação: [Create and manage items online (Square Help)](https://squareup.com/help/us/en/article/5115-create-and-manage-items-online).

### Produção

Na conta **Square real** (produção), cadastre os itens pelo:

- **Square Dashboard** em [squareup.com/dashboard](https://squareup.com/dashboard) → **Items**, ou
- **Square Point of Sale** no celular/tablet.

Depois que os itens existirem no catálogo do Square, o endpoint **GET /api/menu** do seu backend passa a devolvê-los para o frontend.

---

## 2. Backend (`backend/` na raiz do projeto)
